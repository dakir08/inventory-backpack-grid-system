...
public class InventoryTetrisDragDropSystem : MonoBehaviour {
    ...
    private readonly Vector2Int[] offsets = new Vector2Int[] {new(10, 0), new(-10, 0)};
    ...
    private void Update() {
        ...
        if (draggingPlacedObject != null) {
            ...
            InventoryTetris toInventoryTetris = GetInventoryTetrisUnderMouse();
            ...
            List<Vector2Int> gridPositionList = (draggingPlacedObject.GetPlacedObjectTypeSO() as ItemTetrisSO).GetGridPositionList(placedObjectOrigin, dir);

            foreach (InventoryTetris inventoryTetris in inventoryTetrisList) {
                inventoryTetris.ResetVisual();
            }


            if (toInventoryTetris.CheckValidPlacement(draggingPlacedObject.GetPlacedObjectTypeSO() as ItemTetrisSO, placedObjectOrigin, dir)) {
                foreach (Vector2Int gridPosition in gridPositionList) {
                    toInventoryTetris.UpdateValidVisual(gridPosition);
                    // Debug.Log($"${++cnt} : " + "x: " + gridPosition.x + " y: " + gridPosition.y);
                }
            } else {
                foreach (Vector2Int gridPosition in gridPositionList) {
                    
                    toInventoryTetris.UpdateUnValidVisual(gridPosition);
                    // Debug.Log($"${++cnt} : " + "x: " + gridPosition.x + " y: " + gridPosition.y);
                }
            }
    ...
    private InventoryTetris GetInventoryTetrisUnderMouse() {
        foreach (InventoryTetris inventoryTetris in inventoryTetrisList) {
            Vector3 screenPoint = Input.mousePosition;
        ...
        return draggingInventoryTetris;
    }
    ...
    public void StartedDragging(InventoryTetris inventoryTetris, PlacedObject placedObject) {
        ...
        List<Vector2Int> gridPositionList = (placedObject.GetPlacedObjectTypeSO() as ItemTetrisSO).GetGridPositionList(placedObject.GetGridPosition(), dir);
        foreach (Vector2Int gridPosition in gridPositionList) {
            inventoryTetris.GetGrid().GetGridObject(gridPosition.x, gridPosition.y).SetTriggeredObject(true);
        }

        // draggingInventoryTetris.RemoveItemAt(placedObject.GetGridPosition());
    ...
    public void StoppedDragging(InventoryTetris fromInventoryTetris, PlacedObject placedObject) {
        ...
        // Remove triggered object
        List<Vector2Int> gridPositionList = (placedObject.GetPlacedObjectTypeSO() as ItemTetrisSO).GetGridPositionList(placedObject.GetGridPosition(), dir);
        foreach (Vector2Int gridPosition in gridPositionList) {
            fromInventoryTetris.GetGrid().GetGridObject(gridPosition.x, gridPosition.y).SetTriggeredObject(false);
        }
